...
void fix16_to_string(fix16_t n, char * const s)
{
	memset(s,0,20);
	...
	{
		strcat(p++,"-");
		n = -n;//fix16_mul(-1*fix16_one,n);
	}

	itoa(n,temp,2);

	// Get whole part
	int32_t wp = (int32_t)((n&0xFFFF0000)>>16);
	...
	uint32_t fp10 = 0;
	int32_t r[16] = {1,3,6,12,24,49,98,195,391,781,1536,3125,6250,12500,25000,50000};
	for (int i=0; i<16; i++)
	{
		if ((fp>>i) & 0x1)
		{
	...
	const int32_t d[5] = { 10000, 1000, 100, 10 };
	for (int i=0; i<4; i++)
	{
		if (n < d[i])
			strcat(p,"0");
...
fix16_t fix16_from_string(const char * const n)
{
	char s[30];
	strcpy(s,n);
	...
	while (*d != '.')
	{
		...
		if (*d == '\0')
		{
			//UART_Write("WHOLE NUM\n");
			return  sign*(strtol(w,NULL,10) << 16);
		}
		d++;
	}

	// Replace decimal dot with terminator
	*d = '\0';
	d++;

	// Whole part
	int32_t fpw = strtol(w,NULL,10);


	//UART_Write("Fraction: ");
	//UART_Write(d);
	//UART_Write("   Fraction padded: ");
	// Pad fraction part with trailing zeros
	char temp[9] = "00000000";
	char * t = temp;
	while(*d!='\0')	*t++ = *d++;
	uint32_t f = strtol(temp,NULL,10);

	UART_Write(temp);

	// Convert base 10 fraction to fixed point base 2
	uint32_t r[16] = {	50000000,		// 2^-1  = 0.50000000 0
						25000000,		// 2^-2	 = 0.25000000 0
						12500000,		// 2^-3	 = 0.12500000 0
						6250000,		// 2^-4	 = 0.06250000 0
						3125000,		// 2^-5	 = 0.03125000 0
						1562500,		// 2^-6	 = 0.01562500 0
						781250,			// 2^-7  = 0.00781250 0
						390625,			// 2^-8  = 0.00390625 0
						195313,			// 2^-9  = 0.00195312 5
						97656,			// 2^-10 = 0.00097656 2
						48828,			// 2^-11 = 0.00048828 1
						24414,			// 2^-12 = 0.00024414 0
						12207,			// 2^-13 = 0.00012207 0
						6104,			// 2^-14 = 0.00006103 5
						3052,			// 2^-15 = 0.00003051 7
						1526};			// 2^-16 = 0.00001525 8
	uint32_t fpf = 0;
	int32_t bit = 0x8000;
	for (int i=0; i<16; i++)
	{
	...
	char g[15];
	itoa(fpf& 0x0000FFFF,g,2);
	UART_Write("  ");
	UART_Write(g);
	UART_Write("  ");

	// Join fixed point whole and fractional parts
	//fix16_t res = (((sign*fpw)&0x0000FFFF)<<16) + (fpf & 0x0000FFFF);
	fix16_t res =  sign ? -(fpw + fpf) : (fpw + fpf);
	//printLeds32(res);
	return res;
